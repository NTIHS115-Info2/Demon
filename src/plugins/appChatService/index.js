...
module.exports = {
  ...
  async updateStrategy(newMode = 'local', options = {}) {
    ...
    // ───────────────────────────────────────────
    logger.info('[appChatService] 策略更新中...');
    if (!strategies[newMode]) {
      logger.warn(`[appChatService] 不支援的模式 ${newMode}，已切換為 local`);
      newMode = 'local';
    }
    mode = newMode;
    strategy = strategies[newMode];
    this.priority = strategy.priority;
    logger.info(`[appChatService] 策略已切換為 ${mode}`);
  },
  ...
  async online(options = {}) {
    ...
    try {
      ...
      logger.error(`[appChatService] online 失敗: ${err.message || err}`);
      throw err;
    }
  },
  ...
  async offline() {
    ...
    try {
      ...
      logger.error(`[appChatService] offline 失敗: ${err.message || err}`);
      throw err;
    }
  },
  ...
  async restart(options = {}) {
    ...
    try {
      ...
      logger.error(`[appChatService] restart 失敗: ${err.message || err}`);
      throw err;
    }
  },
  ...
  async state() {
    ...
    try {
      ...
      logger.error(`[appChatService] state 查詢錯誤: ${err.message || err}`);
      return -1;
    }
  }
};
