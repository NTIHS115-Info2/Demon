...
class SearchAggregator:
    ...
    def __init__(self, settings_path: Path):
        ...
        self.local_settings_path = settings_path.with_name("setting.local.json")
    ...
    def _load_settings(self) -> Dict[str, str]:
        settings_path = self.local_settings_path if self.local_settings_path.exists() else self.settings_path
        if not settings_path.exists():
            raise FileNotFoundError(f"設定檔不存在: {settings_path}")
        self.loaded_settings_path = settings_path
        logger.info(f"載入設定檔: {settings_path}")
